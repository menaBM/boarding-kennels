...
describe("Routes", ()=>{
    ...
    it("fetches the associated user data of the pet owner", async()=>{
        const response1 = await request(app).get("/pet").set({Authorization: `Bearer ${auth.text}`})
        const response2 = await request(app).get("/pet/fluffy").set({Authorization: `Bearer ${auth.text}`})
    ...
    it("doesn't fetch the pets that don't belong to the user", async()=>{
        const response1 = await request(app).get("/pet").set({Authorization: `Bearer ${auth.text}`})
        const response2 = await request(app).get("/pet/buster").set({Authorization: `Bearer ${auth.text}`})
        const response3 = await request(app).get("/pet/date/2023-04-25").set({Authorization: `Bearer ${auth.text}`})
    ...
    it("allows the user to create a booking for a pet that is then assigned to them", async()=>{
        const response = (await request(app).post("/pet").send({name:"bubbles", age:3, species:"fish", colour:"gold", date_arriving:"2023-05-06", date_leaving:"2023-05-08"}).set({Authorization: `Bearer ${auth.text}`}))
        let pet = await Pet.findOne({where:{name:"bubbles"}})

        expect(response.text).toBe("bubbles has been booked into the kennels")
    ...
    it("allows the user to update their booking", async()=>{
        const response = (await request(app).put("/pet").send({name:"bubbles", date_arriving:"2023-06-06", date_leaving:"2023-06-08"}).set({Authorization: `Bearer ${auth.text}`}))
    ...
    it("does not allow the user to update another users booking", async()=>{
        const response = (await request(app).put("/pet").send({name:"buster", date_arriving:"2023-06-06", date_leaving:"2023-06-08"}).set({Authorization: `Bearer ${auth.text}`}))
    ...
    it("gives the correct error message if the user attempts to update a booking that doesn't exist", async()=>{
        const response = (await request(app).put("/pet").send({name:"dave", date_arriving:"2023-06-06", date_leaving:"2023-06-08"}).set({Authorization: `Bearer ${auth.text}`}))
    ...
    it("does not allow a user to delete another users booking", async()=>{
        const response = (await request(app).delete("/pet").send({name:"buster"}).set({Authorization: `Bearer ${auth.text}`}))

        expect(response.text).toBe("no booking found")
    })

    it("gives the correct error message if the user attempts to delete a booking that doesn't exist", async()=>{
        const response = (await request(app).delete("/pet").send({name:"dave"}).set({Authorization: `Bearer ${auth.text}`}))
...
describe("Accounts", ()=>{
    ...
    it("allows a user to login", async()=>{
        ...
        expect(auth.text).not.toBe(null)
