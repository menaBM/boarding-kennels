...
app.post("/register" ,async(req,res)=>{
    try{
        ...
        let {id, username} = await User.create({username:req.body.username, password:hash})
        let token = jwt.sign({id, username}, process.env.JWT_SECRET)
        res.send(token)
    }catch(error){
        console.log(error)
...
app.put("/pet",authorize, async(req,res)=>{
    try{
        const pet = await Pet.findOne({where:{name:req.body.name, ownerId:req.user.id}})
        if (pet){
...
app.delete("/pet", authorize, async(req,res)=>{
    try{
        let pet = await Pet.destroy({where:{name:req.body.name,ownerId:req.user.id}})
        if (pet){
